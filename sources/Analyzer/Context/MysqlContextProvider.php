...
class MysqlContextProvider implements ContextProvider
{
    ...
    {
        try {
            $result = $this->connection->query("SHOW CREATE SCHEMA " . $this->quoteName($name))->all();
        } catch (ConnectionException $e) {
            if ($e->getCode() === self::SCHEMA_NOT_FOUND) {
                return null;
            } else {
                throw $e;
            }
    ...
    public function getUserDefinition(UserName $name): ?string
    {
        $host = $name->getHost();
        if ($host !== null) {
